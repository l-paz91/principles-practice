...
namespace Graph_lib
{
	...
	void Axis::draw_lines() const
	{
		...
		//add some markers for the notches
		Text zero(Point(m_xy.x - 15, m_xy.y + 15), "0,0");
		zero.draw_lines();
		if (m_o == Axis::y)
		{
			Text yTop(Point(m_xy.x - 20, (m_xy.y - m_length +10)), to_string(m_numNotches));
			yTop.draw_lines();
		}
		if (m_o == Axis::x)
		{
			Text xEnd(Point(m_xy.x + m_length, m_xy.y + 15), to_string(m_numNotches));
			xEnd.draw_lines();
		}
	}
	...
	void Group::draw_lines() const
	{
		...
		//draw pieces
		for (uint32_t i = 0; i < v_light.size(); ++i)
		{
			m_light->draw(int(v_light[i].point(0).x), int(v_light[i].point(0).y));
		}
		for (uint32_t i = 0; i < v_dark.size(); ++i)
		{
			m_dark->draw(int(v_dark[i].point(0).x), int(v_dark[i].point(0).y));
		}
	}
	...
	void Binary_tree::draw_lines() const
	{
		...
		if (!m_useArrowLine)
		{
			for (uint32_t i = 0; i < v_lines.size(); ++i)
			{
				fl_line(int(v_lines[i].point(0).x), int(v_lines[i].point(0).y), 
					int(v_lines[i].point(1).x), int(v_lines[i].point(1).y));
			}
		}
		else
		{
		...
		//draw any text	
		const int fontSize = m_treeSize * 4;
		for (uint32_t i = 0; i < v_nodeText.size(); ++i)
		{
			Text t(v_nodes[v_nodeText[i].node].getPointOnCircle(0), v_nodeText[i].text);
	...
	//very bottom left node is 0, top is v_nodes.size() -1
	void Binary_tree::addTextToNode(uint32_t node, string text)
	{
		if (node < 0 || node > v_nodes.size() - 1)
			error("Cannot have a node less than 0 or more than " + v_nodes.size());
		v_nodeText.push_back(NodeText(node, text));
	}
	...
	Text Test_Controller::show()
	{
		stringstream text;
		text << "Class State : " << m_isClassOn << " || Level: " << m_level;
		return Text(Point(50, 50), text.str());
	}
	
	// -----------------------------------------------------------------------------

	void CustomBinaryTree::addEntry(string label, string parentName, Color color, int indentAmount)
	{
		if (label == parentName)
			error("Cannot be the same unless root node");

		mV_CBTEntries.push_back(CBTentry(label, parentName, color, indentAmount));
	}

	// -----------------------------------------------------------------------------

	void CustomBinaryTree::draw_lines() const
	{
		Point xy(20, 0);
		for (uint32_t i = 0; i < mV_CBTEntries.size(); ++i)
		{
			xy.x = mV_CBTEntries[i].indentX;
			TextBox t(xy, mV_CBTEntries[i].label, 5);
			t.setColours(Color::black, mV_CBTEntries[i].boxColour, Color::black);
			t.draw_lines();
			xy.y += 30;
		}
	}
	...
	BarChart::BarChart(Simple_window* win, vector<double>& heightValues)
		: mv_heightValues(heightValues)
		, m_x(Axis::x, Point(50, win->height() - 50), win->width() - 100, mv_heightValues.size(), "x")
		, m_y(Axis::y, Point(50, win->height() - 50), win->height() - 100, win->height() / 10, "y")
		, m_title(Point{double(win->width() / 2), 20}, "")
	{
		m_barWidth = (win->width() - 100) / mv_heightValues.size();
		m_showBarLabels = m_fillBars = false;
	}
	...
	void BarChart::draw_lines() const
	{
		...
		//bars
		double x = m_x.point(0).x;
		double y = m_y.point(0).y;
		if (m_showBarLabels)
		{
			for (double d : mv_heightValues)
			{
				Graph_lib::Rectangle r(Point{ x, y - d }, m_barWidth, d);
				if (m_fillBars)
					r.set_fill_color(randint(255));
				r.draw_lines();
				Graph_lib::Text t{ Point{x + 5, y - d - 5}, to_string(d) };
				t.draw_lines();
				x += m_barWidth;
			}
		}
	...
	CustomGraph::CustomGraph(Simple_window* win)
		: m_x(Axis::x, Point(50, win->height() - 50), win->width() - 100)
		, m_y(Axis::y, Point(50, win->height() - 50), win->height() - 100)
		, m_title(Point{ double(win->width() / 2), 20 }, "")
	{
		m_x.setLabel("x"); 
		m_y.setLabel("y");
	...
	CustomGraph::CustomGraph(Simple_window* win, string fileName)
		: CustomGraph(win)
	{
		...
		determineScale();
		m_x.setNotches(m_largestX, Axis::x);
		m_y.setNotches(m_largestY, Axis::y);
	}

	// -----------------------------------------------------------------------------

	CustomGraph::CustomGraph(Simple_window* win, vector<xyValues>& xy)
		: CustomGraph(win)
	{
		mv_data = xy;
	...
	void CustomGraph::changeLabels(string xLabel, string yLabel, Color xCol, Color yCol)
	{
		m_x.setLabel(xLabel);
		m_y.setLabel(yLabel);
		m_x.set_color(xCol);
	...
	void CustomGraph::draw_lines() const
	{
		//axes
		m_x.draw_lines();
		m_y.draw_lines();
	...
	void CustomGraph::determineScale()
	{
		m_largestX = mv_data[0].x;
		m_largestY = mv_data[0].y;

		for (uint32_t i = 1; i < mv_data.size(); ++i)
		{
			if (mv_data[i].x > m_largestX)
				m_largestX = mv_data[i].x;
			if (mv_data[i].y > m_largestY)
				m_largestY = mv_data[i].y;
		}
	}

	// -----------------------------------------------------------------------------	

	void ScatterGraph::addDataSet(vector<xyValues>& xy, string label, Color colour)
	{
		ExtraData ed;
		ed.mv_xydata = xy;
		ed.label = label;
		ed.color = colour;
		mv_extradata.push_back(ed);
	}

	// -----------------------------------------------------------------------------

	void ScatterGraph::draw_lines() const
	{
		//data
		double startx = m_x.point(0).x;
		double starty = m_y.point(0).y;
		for (uint32_t i = 0; i < mv_data.size(); ++i)
		{
			double xval = mv_data[i].x * 2;
			double yval = mv_data[i].y * 2;
			Mark m(Point{ m_x.getNotch(xval-1).x, m_y.getNotch(yval-1).y }, 'X', Color::red);
			m.draw_lines();
		}

		//awful
		double textx(10), texty(10);
		if (mv_extradata.size() > 0)
		{
			for (int i = 0; i < mv_extradata.size(); ++i)
			{
				Text t(Point(textx, texty), mv_extradata[i].label);
				t.set_color(mv_extradata[i].color);
				t.draw_lines();
				for (int j = 0; j < mv_extradata[i].mv_xydata.size(); ++j)
				{
					double xval = mv_extradata[i].mv_xydata[j].x * 2;
					double yval = mv_extradata[i].mv_xydata[j].y * 2;
					Mark m(Point{ m_x.getNotch(xval-1).x, m_y.getNotch(yval-1).y }, 'X', mv_extradata[i].color);
					m.draw_lines();
				}
				textx += 50;
			}
