...
namespace Graph_lib 
{
	...
	Window::Window(int ww, int hh, const char* title)
		:Fl_Window(ww, hh, title), w(ww), h(hh)
	{
		init();		
	}
	...
	Window::Window(Point xy, int ww, int hh, const char* title)
		: Fl_Window(int(xy.x), int(xy.y), ww, hh, title), w(ww), h(hh)
	{
		init();
	}
