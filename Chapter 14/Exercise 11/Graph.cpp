...
namespace Graph_lib
{
	...
	{	
		...
		for (int i = 0; i < m_levels - 1; ++i)
		{
			...
			{
				//draw node
				Circle temp(Point(p.x, p.y), 5);
				if ((j & 1) == 0)
				{
					Circle temp2(Point((start.x + (diffX / 2))+(diffX*j), start.y - 20), 5);
					v_lines.push_back(Line(Point(temp.getPointDirection(Directions::N)), Point(temp2.getPointDirection(Directions::S))));
					v_lines.push_back(Line(Point(temp.point(0).x + diffX + 5, temp.point(0).y), Point(temp2.getPointDirection(Directions::S))));
				}
		...
		//draw top node
		v_nodes.push_back(Circle(Point(p.x, p.y), 5));
	}
