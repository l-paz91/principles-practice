...
namespace Graph_lib 
{
	...
	struct Color 
	{
		enum Color_type 
		{
			...
			lime = 71, coral = 130, background2 = FL_BACKGROUND2_COLOR, background = FL_BACKGROUND_COLOR
		};
	...
	struct TextBox : Text
	{
		TextBox(Point p, string msg, int cornerRoundAmount)
			...
			, box(p, length() + double(20.0), font_size() + 20, cornerRoundAmount)
		{}
	...
	*/
	...
	struct Binary_tree : Shape
	{
		...
		Binary_tree(uint32_t levels, int y, NodeType nodeType = NODE_triangle, int treeSize = 1)
			: m_levels(levels)
			, m_offset(y)
			, m_treeSize(treeSize)
		...
		void addTextToNode(uint32_t node, string text);
	...
	// -----------------------------------------------------------------------------
	struct Simple_window;	//forward declare (as definition is in cpp)
	...
	struct CustomGraph : Shape
	{
		...
		{
			xyValues() {}
			xyValues(double dx, double dy) : x(dx), y(dy) {}
			double x, y;
		};

	public:
		CustomGraph(Simple_window* win);							// default constuctor	
		CustomGraph(Simple_window* win, string fileName);			// construct with text file
		CustomGraph(Simple_window* win, vector<xyValues>& xy);		// construct with vector

		void changeLabels(string xLabel, string yLabel, Color xCol = Color::black, Color yCol = Color::black);
		void addChartTitle(string title, Color col = Color::blue) { m_title.set_label(title); m_title.set_color(col); }
		void draw_lines() const;

	protected:
		void determineScale();

		vector<xyValues> mv_data;
		Axis m_x, m_y;
		Text m_title;
		double m_largestX, m_largestY;
	};

	// -----------------------------------------------------------------------------

	struct ScatterGraph : CustomGraph
	{
	public:
		ScatterGraph(Simple_window* win)					   : CustomGraph(win) {}
		ScatterGraph(Simple_window* win, string fileName)	   : CustomGraph(win, fileName) {}
		ScatterGraph(Simple_window* win, vector<xyValues>& xy) : CustomGraph(win, xy) {}

		void addDataSet(vector<xyValues>& xy, string label, Color colour);
		void draw_lines() const;
	...
	private:
		struct ExtraData
		{
			ExtraData()
				: color(0)
			{}
			vector<xyValues> mv_xydata;
			string label;
			Color color;
		};

		vector<ExtraData> mv_extradata;
	};
