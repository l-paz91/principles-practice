...
namespace Graph_lib 
{
	...
	struct Rectangle : Shape 
	{
		...
		//allow a copy contructor for Rectangles
		//Rectangle& operator=(const Rectangle& rect) { h = rect.h; w = rect.w; }
	...
	struct Regular_Hexagon : Polygon
	{
		Regular_Hexagon(Point center, int radius)
		{
			...
			{
				double x = radius * cos(2 * PI * i / numSides) + center.x;
				double y = radius * sin(2 * PI * i / numSides) + center.y;
	...
	struct TextBox : Text
	{
		TextBox(Point p, string msg, int cornerRoundAmount )
			...
			, box(p, msg.size() * font_size(), font_size() + 20, cornerRoundAmount)
		{}
	...
	struct Circle : Shape 
	{
		...
		Point getPointOnCircle(int angle);
