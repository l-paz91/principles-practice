...
//------------------------------------------------------------------------------
//just because I wanted to see if this worked - it does
double getPI()
{
	//obtain PI directly from the FPU (floating point unit) found on the CPU
	double pi;
	//everything in the brackets must adhere to ASM syntax
	__asm
	{
		fldpi		; load PI to the stack (fldpi == fpu load pi)
		fstp pi		; get PI off stack and store in pi variable, then pop the stack to clear it (fstp == fpu store pop)
	}
	return pi;
}
