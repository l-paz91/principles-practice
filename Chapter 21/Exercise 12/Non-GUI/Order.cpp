...
Order::Order(const Order& pOrder)
{
	mName = pOrder.mName;
	mAddress = pOrder.mAddress;
	mDate = pOrder.mDate;
	mPurchases.clear();
	mPurchases.reserve(pOrder.mPurchases.size());
	mPurchases = pOrder.mPurchases;
}
...
{
	mName = order.mName;
	mAddress = order.mAddress;
	mDate = order.mDate;
	mPurchases.clear();
	mPurchases.reserve(order.mPurchases.size());
	mPurchases = order.mPurchases;
...
void Order::readInFromFile(ifstream& readIn)
{
	if (!mName.empty())
	{
...
void Order::printPurchases()
{
	for (Purchase p : mPurchases)
		cout << p << "\t" << p.getPurchaseValue() << endl;
}
